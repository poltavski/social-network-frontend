...
def process_initial(request: Request):
    ...
    if request.cookies:
        user_response = requests.get(
            url=BACKEND_SERVER + "/user/get-current-user", cookies=request.cookies
        )
        ...
        if user_response.status_code == 422:
            ...
            if refresh_r.status_code == 200:
                is_authorized = True
                user_response = requests.get(
                    url=BACKEND_SERVER + "/user/get-current-user",
                    cookies=refresh_r.cookies,
                )
                if user_response.status_code != 200:
                    is_authorized = False
    ...
    if is_authorized:
        ...
        html_response = JINJA_TEMPLATES.TemplateResponse(
            "layout.html", {"request": request, "user_data": user_data, "data": data}
        )
        if refresh_r:
            ...
            if access_token_cookie and csrf_access_token:
                html_response.set_cookie(
                    key="access_token_cookie", value=access_token_cookie
                )
                html_response.set_cookie(
                    key="csrf_access_token", value=csrf_access_token
                )
